	...
	LPARAM	lParam)
{
	...
	case WM_COMMAND:
		...
		case IDOK:
			...
			if(MessageBox(hDlg,"Apply settings?","!!! Warning !!!",MB_YESNO|MB_ICONWARNING)==IDYES)
			#elif	defined(VFD_TRANSLATION_ITALIAN)
			if(MessageBox(hDlg,"Applicare le impostazioni?","!!! Attenzione !!!",MB_YESNO|MB_ICONWARNING)==IDYES)
			#elif	defined(VFD_TRANSLATION_SPANISH)
			if(MessageBox(hDlg,"¿Aplicar la configuración?","!!! Atención !!!",MB_YESNO|MB_ICONWARNING)==IDYES)
			#else
